...
;
;
.alias crc	$38		; CRC lo byte
.alias crch   $39		; CRC hi byte
.alias ptr    $3a		; data pointer
.alias ptrh   $3b		;   "    "
.alias blkno  $3c		; block number
.alias retry  $3d		; retry counter
.alias retry2 $3e		; 2nd counter
.alias bflag  $3f		; block flag 
.alias reclen $39		; record length in bytes
.alias chksum crc		; record checksum accumulator
.alias start_lo      $3b
.alias start_hi	     $3c
.alias rectype		     $3d
.alias dlfail $3e		; flag for upload failure
.alias temp   $3f		; save hex value
.alias strptr $40
.alias strptrh	      $41		; temporary string pointer (not preserved across calls)
...
.alias Rbuff		$0300		; temp 128 byte receive buffer 
		; (uses the Monitor's input buffer)
.alias SOH		$01		; start block
.alias EOT    $04		; end of text marker
.alias ACK    $06		; good block acknowleged
.alias NAK    $15		; bad block acknowleged
.alias CAN    $18		; cancel (not standard, not supported)
.alias CR     13
.alias LF     10
.alias ESC    27		; ESC to exit
.alias Colon  $3a
.alias Semi   $3b
.alias Space  $20
.alias Pound  $23
...
GotByte3:	cmp #SOH		; start of block?
	beq BegBlk		; yes
	cmp #Colon		; Intel-Hex format - jump to its handler below
	bne GotByte1		; 
	jmp HexUpLd		; 
GotByte1:	cmp #EOT		;
...
HdLp1:	jsr GetHex		; Get the first/next/last data byte
	...
	lda #Pound		; Character indicating record OK = '#'
