...
; 2014-01-31 Modifications to add IEC-related commands by Richard Cini
...
; 2.22A	Formatting changes for 8-space tab stops
;	Added "BYE" command to return to the monitor
;		Add token# to correct token table TK_
;		Add address to command vector table LAB_CTBL
;		Add key word string to appropriate key word table LBB_
;		Add keyword letter to appropriate LIST decode table TAB_ASC*
;	Serial IEC changes
...
Itemph		= Itempl+1	; 12 temporary integer high byte

nums_1		= Itempl	; 11 number to bin/hex string convert MSB
nums_2		= nums_1+1	; 12 number to bin/hex string convert
nums_3		= nums_1+2	; 13 number to bin/hex string convert LSB
...
OUTDEV 		= $1f	;output device (default 3=SCREEN)
					; ** use this to control LIST/LLIST output
					; ** by trapping character output
...
LPVP		= $2f	;{*}POINTER: existing output vector (trapped by LLIST)
TMP2		= $31	;used for JOY(0)
CSRMOD		= $32	;cursor mode
SAL 		= $33	;{*}POINTER: start address
EAL 		= $35	;{*}POINTER: end address/end of program
FN1P		= $37	;index into filename #1 string
FN2P		= $38	;index into filename #2 string
CMD_DEV	 	= $39	;DOS device# for current command
;
; $3A- $5A available
;
...
TK_STAT		= TK_MON+1		; STAT token
TK_CATALOG	= TK_STAT+1		; CATALOG token
TK_DRV		= TK_CATALOG+1		; DRV (default drive) token
TK_INIT		= TK_DRV+1		; INIT token
TK_VALIDATE	= TK_INIT+1		; VALIDATE token
TK_SCRATCH	= TK_VALIDATE+1		; SCRATCH token
TK_FORMAT	= TK_SCRATCH+1		; FORMAT token
TK_RENAME	= TK_FORMAT+1		; RENAME token
TK_COPY		= TK_RENAME+1		; COPY token
TK_LLIST	= TK_COPY+1		; LLIST token
TK_OPEN		= TK_LLIST+1		; OPEN token
TK_CLOSE	= TK_OPEN+1		; CLOSE token
TK_INPUTN	= TK_CLOSE+1		; INPUT# token
TK_PRINTN	= TK_INPUTN+1		; PRINT# token

; secondary command tokens, can't start a statement
TK_TAB		= TK_PRINTN+1		; TAB token
...
TK_JOY		= TK_MIDS+1		; JOY token	NEW
...
Ibuffs		= VEC_SV+$16	; start of input buffer after IRQ/NMI code
Ibuffe		= Ibuffs+$47	; end of input buffer $0268
; CAN DELETE WHEN DONE CODING
AA_bufend1	.word Ibuffe
;
;  Last buffer address is $0268 so addresses below should work fine.
;
...
FNM2e		=VarStart+39
; CAN DELETE WHEN DONE CODING
AA_bufend2	.word FNM2e	; $02B5

; $0300 page is used by SBCOS
Ram_base	= $0400	; start of user RAM (set as needed, should be page aligned)
Ram_top		= $7F00	; end of user RAM+1 (set as needed, should be page aligned)
...
;*****
	clv
	bvc	LAB_DOMEM		; skip memory input and assume all mem
;*****
...
;*****
	LDA	#<LAB_SMSG2		; point to sign-on message (low addr)
	LDY	#>LAB_SMSG2		; point to sign-on message (high addr)
;*****
	JSR	LAB_18C3		; print null terminated string from memory
...
LAB_SMSG
	.byte	$1B,"[2J**** Enhanced BASIC V2.3 ****",$0D,$0A
	.byte	       "    BASIC by Lee Davison",$0D,$0A
	.byte	       " IEC Support by Richard Cini",$0D,$0A,$00
LAB_SMSG2	
...
LAB_CTBL
	...
	.word	LAB_SYSjmp-1		; MON		new command
	.word	LAB_STAT-1		; STATUS	new command
	.word	LAB_CATALOG-1		; CATALOG	new command
	.word	LAB_DRIVE-1		; DRV		new command
	.word	LAB_INIT-1		; INIT		new command
	.word	LAB_VALIDATE-1		; VALIDATE	new command
	.word	LAB_SCRATCH-1		; SCRATCH	new command
	.word	LAB_FORMAT-1		; FORMAT	new command
	.word	LAB_RENAME-1		; RENAME	new command
	.word	LAB_COPY-1		; COPY		new command
	.word	LAB_LLIST-1		; LLIST		new command
	.word	LAB_OPEN-1		; OPEN		new command
	.word	LAB_CLOSE-1		; CLOSE		new command
	.word	LAB_INPUTN-1		; INPUT#	new command
	.word	LAB_PRINTN-1		; PRINT#	new command
...
LAB_FTPM	= LAB_FTPL+$01
	...
	.word	LAB_LRMS-1		; MID$()		"
	.word	LAB_1BF7-1		; JOY(x)	process any expression
...
LAB_FTBM	= LAB_FTBL+$01
	...
	.word	LAB_MIDS-1		; MID$()
	.word	LAB_JOY-1		; JOY()		new function
...
LBB_CATALOG
	.byte	"ATALOG",TK_CATALOG	; CATALOG
LBB_CHRS
	.byte	"HR$(",TK_CHRS		; CHR$(
LBB_CLEAR
...
LBB_INC
	.byte	"NC",TK_INC		; INC
LBB_INIT
	.byte	"NIT",TK_INIT		; INIT
LBB_INPUTN
	.byte	"NPUT#",TK_INPUTN	; INPUT#
LBB_INPUT
...
LBB_SCRATCH
	.byte	"CRATCH",TK_SCRATCH	; SCRATCH
LBB_SGN
	.byte	"GN(",TK_SGN		; SGN(
LBB_SIN
	.byte	"IN(",TK_SIN		; SIN(
...
LBB_VAL
	.byte	"AL(",TK_VAL		; VAL(
LBB_VALIDATE
	.byte	"ALIDATE",TK_VALIDATE	; VALIDATE
LBB_VPTR
...
LAB_KEYT
	...
	.word	LBB_STAT	; STAT
	.byte	7,"C"
	.word	LBB_CATALOG	; CATALOG
	.byte	5,"D"
	.word	LBB_DRIVE	; DRV
	.byte	4,"I"
	.word	LBB_INIT	; INIT
	.byte	8,"V"
	.word	LBB_VALIDATE	; VALIDATE
	.byte	7,"S"
	.word	LBB_SCRATCH	; SCRATCH
	.byte	6,"F"
	.word	LBB_FORMAT	; FORMAT
	.byte	6,"R"
	...
	.byte	6,"I"
	.word	LBB_INPUTN	; INPUT#
	.byte	6,"P"
	.word	LBB_PRINTN	; PRINT#
...
; BASIC messages pointer table, mostly error messages
LAB_BAER
...
; dimensioning of arrays before use.
	.word	ERR_UV		;$24 undefined variable	tested and works (see code and comments below LAB_1D8B)
	.word	ERR_UA		;$26 undimensioned array
	.word	ERR_T0		;$28 too many files
	.word	ERR_FO		;$2A file open
	.word	ERR_FNO		;$2C file not open
	.word	ERR_FNF		;$2E file not found
	.word	ERR_DNP		;$30 device not present
	.word	ERR_NIF		;$32 not input file
	.word	ERR_NOF		;$34 not output file
	.word	ERR_MFN		;$36 missing filename
	.word	ERR_IDN		;$38 illegal device number
	.word	ERR_BDC		;$3A bad command
	.word	ERR_BDP		;$3C bad parameter
...
ERR_UV	.byte	"Undefined variable",$00	; tested and works (see code LAB_1D8B)
ERR_UA	.byte	"Undimensioned array",$00
;new
ERR_T0	.byte	"Too manu files",$00
ERR_FO	.byte	"File open",$00
ERR_FNO	.byte	"File not open",$00
ERR_FNF	.byte	"File not found",$00
ERR_DNP	.byte	"Device not present",$00
ERR_NIF	.byte	"Not input file",$00
ERR_NOF	.byte	"Not ooutput file",$00
ERR_MFN	.byte	"Missing filename",$00
ERR_IDN	.byte	"Illegal device number",$00
ERR_BDC .byte	"Bad command",$00
ERR_BDP .byte 	"Bad parameter",$00
...
;new
LAB_SRCM	.byte	"Searching for ",$00
;LAB_FORR	.byte	"for ",$00
LAB_LDMS	.byte 	"Loading ",$00
LAB_WRMS	.byte 	"Writing ",$00
LAB_VFMS	.byte 	"Verifying ",$00
LAB_FNDM	.byte 	"Found ",$00
LAB_SVMS	.byte	"Saving ",$00
LAB_OKMS	.byte 	"OK",$00
LAB_SURE	.byte 	"Are you sure?",$00
LAB_BADD	.byte 	"Bad disk",$00
...
;***** Commands added by RAC
;
; LAB_SYSjmp - go to the monitor
LAB_SYSjmp
	JMP	Monitor


;===============================================================
; JOY(x) - Read the joystick port and return it to BASIC
;	   The value of 'x' is not meaningful and is ignored.
;
;	   Return JOY as F000RLDU
;
;	PortA shared with IEC...PA7.PA1.PA0
;===============================================================
LAB_JOY
	LDA	Via1PRA1	; get PortA, no handshaking
	CMP	Via1PRA1
	BNE	LAB_JOY		; wait for it to settle
	
	PHA			; save bitmask
	AND	#%00111100	; mask of JOYs
	LSR			; move down 2
	LSR
	STA	TMP2		; save return value
	PLA			; get back original bitmap
	AND	#%01000000	; mask for fire button
	CMP	#%01000000	; check if fire pushed...1=open 0=pushed
	BNE	LAB_JOYE	; yes, pushed
	LDA	TMP2		; get return value
	ORA	#%10000000	; set bit7 for not pushed
	STA	TMP2		; save it
	
LAB_JOYE
	LDA	TMP2		; get final bitmask
	EOR	#%10001111	; flip bits for positive logic
	TAY			; save to Y
	LDA	#$00		; clear A
	JMP	LAB_AYFC	; return it to BASIC


;===============================================================
; LLIST - new command to do a LIST to device 4
;	This command doesn't exist in the VIC ROM but we try
;	to emulate the sequence "OPEN4,4:CMD4:LIST:CLOSE4"
;	by trapping the character output routine in the
;	existing LIST command. Command takes the same parameters
;	as LIST. 
;
;===============================================================
LAB_LLIST
	...
	bit 	CSTAT		; is it there?
	bmi 	LLIST_DNP	; no, exit
	
;	lda 	#$00		; 00= graphics/upper case
	lda 	#$07		; 07= upper/lower case
	and	#$60
	jsr 	SECOND
	bit 	CSTAT
	bpl 	GOLIST
...
LLIST_DNP
	...
	tax
	pla
	LDX	#$30		; "device not present" error
	JMP	LAB_XERR	; jump to error routine
...
;***** Disk Commands
;===============================================================
; STAT- STAT[,device] where device is 8-11. Reads the drive
;	status and prints it to the console.
; 
;===============================================================
LAB_STAT		; STATUS
	...
	jsr LAB_GBYT	; any drive# parameter?
	beq DOSTAT	; no, process command

	jsr LAB_SCGB	; yes, param...check for "," and get to X
	cpx #$08	; Below 8?
	bmi DSERR	; yes...error
	cpx #$0b	; above 11
	bcs DSERR	; yes...error
...
DOSTAT			; we have a valid unit number, so get status
	...
	LDX #$ff	; set channel for input $0F??
	JSR CHKIN
...
LINE	JSR CHRIN	;get BASIC line# LSB (#blocks)	
	...
	JSR LAB_295E	; in BASIC print XA as unsigned integer
...
C2			; EOL reached
	jsr LAB_CRLF	; print CRLF
	JSR CHRIN	; get next char (s/b nxt_ptr_L or 0)
	BNE BCK1	; If not 0, get next char (nxt_ptr_H)
	JSR CHRIN	; Got first 0, so check for second. Two nulls means end of dir
...
;===============================================================
; VALIDATE - VALIDATE[,device] where device is 8-11
;
;===============================================================
LAB_VALIDATE		; VALIDATE
	...
	jsr LAB_GBYT	; any drive# parameter?
	beq COL0	; no, process command

	jsr LAB_SCGB	; yes, param...check for "," and get to X
	cpx #$08	; Below 8?
	bmi CERROR	; yes...error
	cpx #$0b	; above 11
	bcs CERROR	; yes...error
	stx CMD_DEV	; overwrite default device
...
COL0
	...
	JSR CHKOUT
	JSR READST
	lda #"V"	; $56 select validate command
...
INIT0
	...
	JSR CHKOUT
	JSR READST
	lda #"I"	; $49 select initialize command
	JSR CHROUT	; send command to channel
	JSR CLRCH
	LDA #$ff	; Now close the file
	JSR CLOSE
...
;===============================================================
; DRIVE - DRIVE[,device] where device is 8-11
;	sets drive# for next DOSCMD operation
;
;===============================================================
LAB_DRIVE
	ldx 	#$08		; default to unit 8
	stx 	CMD_DEV		; save default device for current command
	jsr 	LAB_GBYT	; any drive# parameter?
	beq 	DRV0		; no, assign drive 8

	jsr 	LAB_SCGB	; yes, param...check for "," and get to X	
	cpx 	#$08		; Below 8?
	bmi 	DRVERRF		; yes...error
	cpx 	#$0b		; above 11
	bcs 	DRVERRF		; yes...error
	stx 	CMD_DEV		; overwrite default device
DRV0
	rts
...
SCR15
	lda 	SNEW,x		; Copy drive command to buffer
	beq 	SCR20
	sta 	FNM1,x
	inx
	bra 	SCR15
...
SCR20
	jsr	FillFNBuffer	; get filename and... 
				; send command string
	ldy	#$00		; reset index
SCR30
	lda	FNM1,Y		; get byte
	beq	SCR40		; 0-terminator?
	jsr	CIOUT		; no, send the char to the drive
	iny			; increment index
	bra	SCR30		; loop if not done yet
	
SCR40
	jsr	UNLISTEN	; command device to unlisten
	rts			; exit

SCRERR	
	jmp 	LAB_SNER	; syntax error
...
;===============================================================
; FORMAT - FORMAT[,device] "filename,ID" where device is 8-11.
;	Filename can be up to 16 characters, although string
;	  length test is not yet performed.
;	This is the Commodore DOS "NEW" command.
;===============================================================
LAB_FORMAT
...
FMT30
	lda	FNM1,Y		; get byte
	beq	FMT40		; 0-terminator?
	jsr	CIOUT		; no, send the char to the drive
	iny			; increment index
	bra	FMT30		; loop if not done yet
...
;===============================================================
; RENAME - RENAME[,device] "new=old" where device is 8-11.
;	Filename can be up to 16 characters, although string
;	  length test is not yet performed.
;===============================================================
LAB_RENAME
...
REN1				; device exists
	ldx 	#$00		; Start at beginning of command buffer
REN15
	lda 	SREN,x		; Copy drive command to buffer
	beq 	REN20
	sta 	FNM1,x
	inx
	bra 	REN15
...
REN30
	lda	FNM1,Y		; get byte
	beq	REN40		; 0-terminator?
	jsr	CIOUT		; no, send the char to the drive
	iny			; increment index
	bra	REN30		; loop if not done yet
...
;===============================================================
; COPY - COPY[,device] "backup=original" where device is 8-11.
;	Filename can be up to 16 characters, although string
;	  length test is not yet performed.
;===============================================================
LAB_COPY
...
CPY1				; device exists
	ldx 	#$00		; Start at beginning of command buffer
CPY15
	lda 	SCPY,x		; Copy drive command to buffer
	beq 	CPY20
	sta 	FNM1,x
	inx
	bra 	CPY15
...
CPY30
	lda	FNM1,Y		; get byte
	beq	CPY40		; 0-terminator?
	jsr	CIOUT		; no, send the char to the drive
	iny			; increment index
	bra	CPY30		; loop if not done yet
...
;===============================================================
; FillFNBuffer
;	Routine parses BASIC command line, copies filename
;	into DOS command buffer. Used for Scratch.
;
;===============================================================
FillFNBuffer
	ldx	#$00
	ldy	#$03		; start after "?0:" command
...
FFNB1			
	lda	ibuffs+1,x	; grab a character from command line
	stz	ibuffs+1,x	; this fixes syntax error in ehbasic
	beq	FFNB3		; done copying name to buffer...continue

FFNB15
;	jsr	IsValidChar
	jsr	IsValidCmdStr	; ensure it's an alphanumeric character
	bcc	FFNB2		; not alpha, skip it
	sta	FNM1,y		; starts at $028e
	iny
	inx
	cpy	#$14		; 16 characters + 3 for command
	bcc	FFNB1
	bra	FNBERR1		; entry too long, error
	
FFNB2
	inx			; go to next char in command buffer
	bra	FFNB1

FFNB3				; this is where all the saving occurs
	lda	#$00		; zero-terminate the string for printing
;	iny
	sta	FNM1,y
	dey			; remove 0-terminator from count
	sty	FN1P
	rts
...
; OPEN: OPEN file#,device#[,secondary_addr][,"filename"]
; CLOSE: close logical file# specified as parameter
; INPUT#: INPUT# file#,variable_list
; GET#:	  GET# file#,variable	get single character into variable
; PRINT#: PRINT# file#,[standard print options]
;===============================================================
LAB_OPEN		; OPEN
LAB_CLOSE		; CLOSE
LAB_INPUTN		; INPUT#
LAB_PRINTN		; PRINT#
	LDX	#$3A		; error code $3A ("Bad command" error)
	JMP	LAB_XERR	; jump to error routine
